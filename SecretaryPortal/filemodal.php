<?php
    require_once('../secclass.php');
    $sessionData = $payroll->getSessionSecretaryData();
    $payroll->verifyUserAccess($sessionData['access'], $sessionData['fullname'],2);
    $fullname = $sessionData['fullname'];
    $access = $sessionData['access'];
    $id = $sessionData['id'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Generate Deduction Modal</title>
    <link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.15.4/css/all.css" integrity="sha384-DyZ88mC6Up2uqS4h/KRgHuoeGwBcD4Ng9SiP4dIRy0EXTlnuz47vAwmeGwVChigm" crossorigin="anonymous">
    <link rel="stylesheet" type="text/css" href="../seccss/main.css">
    <script src="https://kit.fontawesome.com/a076d05399.js" crossorigin="anonymous"></script>
</head>
<body>
<div class="main-containter">
    <div class="modal">
        <form action="" method="post" class=modal__form>
             <div class="modal__form__header1">
                    <h1>Download Merged Payslip</h1>
             </div>
                <div class="modal__form__content">
                    <div class="modal__form__content__spaces">
                        <table>
                            <thead>
                                <th class ="file-modal"><h2>Files</h2></th>
                            </thead>
                            <tbody>
                                <?php
                                    $dir = "../SecretaryPortal/merged";
                                    // set file pattern
                                    $dirpath = $dir."/*.pdf";
                                    // copy filenames to array
                                    $files = array();
                                    $files = glob($dirpath);
                                    // sort files by last modified date
                                    usort($files, function($x, $y) {
                                        return filemtime($x) < filemtime($y);
                                    });
                                    
                                        $indexCount=count($files);
                                        // rsort($dirarray);
                                        for($index=0; $index < $indexCount; $index++) 
                                        {
                                            $filename = explode("/",$files[$index]);
                                            if ($files[$index] != "." && $files[$index] != "..") {
                                                echo "<tr><td>
                                                <i class='fas fa-file-pdf' style='font-size:19px;'></i>&emsp;<a href='download.php?file=".$files[$index]."'>".end($filename)."</a></td></tr>\n";
                                            }
                                        }
                                
                                ?>
                            </tbody>
                        </table>
                    </div>
                </div>
                    <center><a href="releasedsalary.php"><button class="cancel" type="button">
                    Back</button></a></center>
        </form>
    </div>
</div>
</body>
</html>

